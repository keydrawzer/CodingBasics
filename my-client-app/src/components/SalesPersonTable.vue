    ...
    <table v-if="state.persons && state.persons.length > 0" class="box-shadow">
        ...
        <tbody>
            <tr v-for="person in paginatedPersons" :key="person.id">
                ...
                <td>{{ person.salesTerritory ? person.salesTerritory : 'N/A' }}</td>
                <td>{{ integerFormatter.format(person.salesCount) }}</td>
                <td>{{ currencyFormatter.format(person.salesYTD) }}</td>
                <td>{{ person.salesQuota ? currencyFormatter.format(person.salesQuota) : 'N/A' }}</td>
                <td>{{ currencyFormatter.format(person.salesTotal) }}</td>
            </tr>
        </tbody>
        <tfoot>
            <tr>
